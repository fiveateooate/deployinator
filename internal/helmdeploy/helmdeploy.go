...
import (
	...
	"github.com/fiveateooate/deployinator/internal/apphandler"
...
func doDeploy(app *apphandler.App) {
...
// HelmDeploy - deploy stuff using helm
func HelmDeploy(msg *pb.DeployMessage) error {
	// var (
	...
	helmRepo := "https://artifacts.weavelab.xyz"
	...
	context := "DSfasdf"
	helmVersion := "ADsfadf"
	clientset, err := k8sbuddy.InClusterClient()
	if err != nil {
		return err
	}
	app := apphandler.App{K8sApp: &k8sApp, HelmInfo: &helmInfo, DeployerType: "helm"}

	k8sApp.GetAppInfo(msg.Slug, msg.Domain, clientset)
	helmInfo.GetHelmInfo(msg.Slug, msg.Domain, helmRepo, helmValues, context, helmVersion)
	doDeploy(&app)
	color.Printf("@cDone\n")
	return nil
}
